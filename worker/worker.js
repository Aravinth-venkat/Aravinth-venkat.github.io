...
3. If the resume does not prove a claim, say "Not established by the resume." Then explain what the candidate should verify.
...
16. When explaining a generic architecture, explicitly label it as: "GENERIC REFERENCE ARCHITECTURE".
...
function originAllowed(env,origin){
  return !env.ALLOWED_ORIGIN||env.ALLOWED_ORIGIN==="*"||env.ALLOWED_ORIGIN===origin;
}
  ...
  if(Array.isArray(data.choices)){
    for(const choice of data.choices){
      ...
      if(Array.isArray(message.content)){
        const text=message.content.map(x=>x?.text||"").filter(Boolean).join("\n").trim();
        if(text)return text;
      }
    }
  }
...
Rules:
- score: integer 0-100
- use only information proven by the resume
- never invent skills, employers, projects, dates, tools or responsibilities
- keep each array to a maximum of 6 short items
- if something is unclear, say "Not established by the resume"
- this is career coaching, not a hiring decision
